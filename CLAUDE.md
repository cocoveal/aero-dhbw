...
- **Localization pattern:** strings (`outline-title`, `bib-title`, AI-usage labels, …) are assigned in an `if text-lang == "en" { ... } else { ... }` block near the top, then referenced below. `titlepage.typ` and `declaration.typ` instead duplicate their *entire body* across EN/DE branches — when you change layout in one branch, mirror it in the other.
