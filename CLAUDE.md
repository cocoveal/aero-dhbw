...
- **Authors:** `author` is polymorphic — a name string (single author, with top-level `mat-number`/`course-acronym`) or an array of `(name, mat-number, course-acronym)` dicts (multiple, **max 6** — `aero-dhbw()` asserts `1..6`). It's normalized via `normalize-authors` into a list of dicts that `titlepage`/`declaration` consume: per-author ID rows + stacked names under "by"/"von"; plural declaration with a signature per author (single = stacked block, 2+ = two-column grid). The title page uses **flexible `v(1fr)` glue** so the cover fits one page regardless of author count.
